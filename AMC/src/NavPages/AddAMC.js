...
//         setPopup({ message:"There was an error deleting the AMC.", isOpen: true, isError:true});
//       }
//     }
//   };
...
export default function AddAmc() {
  ...
  const [tableData, setTableData] = useState([]);
  const [popup, setPopup] = useState({ message: "", isOpen: false, isError: false });
  const [deletePopup, setDeletePopup] = useState({ isOpen: false, amcId: null });
  ...
  const handleDelete = (amcId) => {
    setDeletePopup({ isOpen: true, amcId }); // Open the popup and store the amcId
  };
  
  const confirmDelete = async () => {
    try {
      await axios.delete(`http://localhost:8080/api/addAmcs/delete/${deletePopup.amcId}`);
      setTableData(tableData.filter(item => item.amcId !== deletePopup.amcId)); // Remove the deleted item from the state
      setDeletePopup({ isOpen: false, amcId: null }); // Close the popup
    } catch (error) {
      console.error("Error deleting AMC:", error);
      setPopup({ message: "There was an error deleting the AMC.", isOpen: true, isError: true });
    }
  ...
  const cancelDelete = () => {
    setDeletePopup({ isOpen: false, amcId: null }); // Close the popup without deleting
    window.location.reload();
  };
...
{deletePopup.isOpen && (
  <div className="delete-popup">
    <div className="delete-popup-content">
      <p>Are you sure you want to delete this AMC?</p>
      <button onClick={confirmDelete} className="confirm-button2">Yes</button>
      <button onClick={cancelDelete} className="cancel-button2">Cancel</button>
    </div>
  </div>
)}
